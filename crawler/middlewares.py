...
class ProxyMiddleware(object):
    ...
    def process_request(request, spider):
        ...
        if request.url.startswith("http://"):
            proxy = PROXIES.get("http")  # http代理
        elif request.url.startswith("https://"):
            proxy = PROXIES.get("https")  # https代理
        request.meta['proxy'] = proxy
        # spider.logger.info("代理IP : {}".format(proxy))
...
#     default_argument = [
#         # '-headless',
#         "--user-agent={}".format("Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) "
...
#         driver_arguments += cls.default_argument
